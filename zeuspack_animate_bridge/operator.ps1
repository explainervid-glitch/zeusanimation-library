# ============================================================
# ZeusPack Bridge - Installer Script (PowerShell)
# Installs the CEP panel that links Adobe Animate to ZeusPack.
...
$PluginName   = "ZeusPack Bridge"          # shown to the user
$SourceFolder = "animate_bridge"           # folder next to this script
$InstallName  = "zeuspack_animate_bridge"  # folder name under CEP\extensions
...
if (-not $isAdmin) {
    ...
    # the elevated (UAC) session — elevation runs under a separate
    # logon session that doesn't inherit mapped drives. This means
    # PowerShell can't find '<drive>:\...\operator.ps1' after RunAs,
    # and elevation silently fails with "term not recognized".
    #
    # Fix: copy the ENTIRE project folder (install.bat, operator.ps1,
    # and the animate_bridge\ plugin folder) to a local temp folder
    # BEFORE elevating, then elevate and run from there. The elevated
    # session never needs to touch the network drive at all.
    # ------------------------------------------------------------
    $localPreStage = "$env:TEMP\ZeusPackBridge_preelevate"
    ...
    # The temp file has no spaces, so -File works perfectly.
    $tempLauncher = "$env:TEMP\ZeusPackBridge_elevate_launcher.ps1"
...
"@
    ...
    # The elevated process (and the install it ran) is fully done now.
    # Safe to clean up the pre-elevation staging folder from here, since
    # THIS process is not running from inside that folder.
...
Write-Host "   ZeusPack Bridge - Installer for Adobe Animate   " -ForegroundColor Cyan
Write-Host "  ==================================================" -ForegroundColor DarkCyan
Write-Host ""
...
# When elevated via -Command, $PSScriptRoot is empty.
# Fall back to the directory of the resolved path used above.
if (-not $scriptDir) {
...
# ============================================================
# By this point $sourceDir already points to a local temp folder
# (copied there before the elevation request), so no NAS access is
# needed from here on. This block just confirms that.
Write-Step "Confirming plugin source is local..."
Write-Ok "Running from local staged copy: $sourceDir"

# ============================================================
# 4. CHECK ADOBE ANIMATE PROCESS
# ============================================================
...
if ($animateProcess) {
    ...
    switch ($choice) {
        ...
        "2" {
            Write-Warn "Continuing without closing. Restart Animate after installation is complete."
        }
        default {
...
if ($allOk) {
    ...
    Write-Host "  2. Open Adobe Animate 2024"                          -ForegroundColor Gray
    ...
    # Clean up the local pre-elevation staging copy is handled by the
    # parent (non-elevated) process after this elevated session exits,
    # since this script is currently running from inside that folder.
} else {
